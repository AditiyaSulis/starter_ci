...
class m_attendance extends CI_Model
{
	...
	public function totalTelatByAttendance_get($id, $tanggal1, $tanggal2)
	{
		$count = $this->db
			->where('id_employee', $id)
			->where('status', 2)
			->where('time_management', false)
			->where('tanggal_masuk >=', $tanggal1)
			->where('tanggal_masuk <=', $tanggal2)
	...
	public function getAttendanceDataCore_get()
	{
		...
		$this->db->select('attendance.id_attendance, attendance.id_employee, attendance.id_schedule,  attendance.jam_masuk, attendance.status, attendance.tanggal_masuk, attendance.time_management, attendance.potongan_telat,  schedule.id_workshift, schedule.waktu, workshift.clock_in, workshift.clock_out,  workshift.name_workshift, employee.name, employee.id_product, products.name_product');
		if($id != 'false') {
