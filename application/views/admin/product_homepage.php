...
<main>
				...
				<table id="products_table" class="table table-bordered table-striped" style="width:100%">
					...
					<?php foreach ($products as $product): ?>
						<tr>
							...
							<td>
								<img src="<?= base_url('uploads/product_homepage/' . $product['logo']); ?>" alt="Logo" width="50" style="cursor: pointer;" onclick="showImageModal('<?= base_url('uploads/product_homepage/' . $product['logo']); ?>')">
							</td>
							<td>
								<button
									class="btn gradient-btn-edit btn-sm mb-2 rounded-pill btn-edit" style="width : 70px"
									data-id="<?= $product['id_product_homepage']; ?>"
								...
								<button class="btn gradient-btn-delete mb-2 btn-sm rounded-pill btn-delete-product-homepage" data-id_product="<?= $product['id_product_homepage']; ?>" style="width : 70px">
									DELETE
								</button>
							</td>
						</tr>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form w-100" id="addproduct" data-action="<?= site_url('admin/product_homepage/add_products') ?>" enctype="multipart/form-data">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form w-100" id="editProductHomePageForm">
						<input type="hidden" id="edit_id" name="id_product_homepage">
						<div class="mb-3">
	...
	<script>
		...
		$("#editProductHomePageForm").on("submit", function (e) {
			...
			$.ajax({
				url: base_urls +"admin/product_homepage/update",
				type: "POST",
				data: formData,
				contentType: false,
				processData: false,
			...
			$(".btn-delete-product-homepage").on("click", function () {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						$.ajax({
							url: base_url + "admin/product_homepage/delete",
							type: "POST",
